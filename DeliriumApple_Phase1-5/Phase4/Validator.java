...
public class Validator {
	 ...
	 */
	public String withdrawal(float balance, float transactionFee, char type, float transactionAmount, int accNum, String command) {
	
		//Determine what amount to transfer
		balance = balance + -(transactionAmount + transactionFee);
	 ...
	 * @return a string either pass, fail, or fatal to show results of validation
	 */
	public String deposit (float balance, float transactionFee, char type, float transactionAmount, int accNum , String command) {
		//Determine what amount to transfer
		balance = balance + (transactionAmount + transactionFee);
	 ...
	 * @return a string either pass, fail, or fatal to show results of validation
	 */
	public String paybill (float balance, float transactionFee, char type, float transactionAmount, int accNum, String mm) {
		if(!((mm.equals("EC"))  || (mm.equals("TV"))   || (mm.equals("CQ")))){
			System.out.println((mm.equals("EC")) + " 1" + mm + "1 " );
			System.out.println("ERROR: Paybill company " + mm + " does not exist.");
			return fatal;
		} 
		if (!(withdrawal(balance, transactionFee, type, transactionAmount, accNum, "Paybill").equals("Pass"))) {
			return fail;
		}
		return pass;
	}
	...
	/**
	 * Verifies a given user input, by checking that the name exists, the account number exists, and that the name matches the account number
	 * @param accounts list of all accounts
	 * @param accNum The account number
	 * @param accName The account name
