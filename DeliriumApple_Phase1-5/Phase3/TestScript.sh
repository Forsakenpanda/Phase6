...
folders=( "INITIALCOMMANDS" "LOGIN" "WITHDRAW" "DEPOSIT" "PAYBILL" "TRANSFER" "CHANGEPLAN" "ENABLE" "DISABLE" "CREATE" "DELETE" )

# Loop through each folder
for folder in "${folders[@]}"
do
     # Loop through each file in the directory
     for file in `find ./INPUT/$folder -name "*.txt" -type f | sort -r`
     do
          ...
          if [ -z "$outputresult" ] && [ -z "$transactionresult" ]; then
